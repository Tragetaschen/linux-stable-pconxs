...
static ssize_t roi_show(struct device *dev, struct device_attribute *attr,
			char *buf, int offset)
{
...
static size_t roi_store(struct device *dev, struct device_attribute *attr,
			const char *buf, size_t count, int offset)
{
	int ret;
	int value_x0, value_x1, value_scale, value_bgscale;
...
static ssize_t fir_show(struct device *dev, struct device_attribute *attr,
			char *buf, int offset)
{
	int values[13];
	int i;
...
static size_t fir_store(struct device *dev, struct device_attribute *attr,
			const char *buf, size_t count, int offset)
{
	int ret, i;
	int values[13];
...
static ssize_t coefficients_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	int c0, c1;
...
static ssize_t weights_show(struct device *dev, struct device_attribute *attr,
			    char *buf)
{
...
static ssize_t weights_store(struct device *dev, struct device_attribute *attr,
			     const char *buf, size_t count)
{
	int ret;
	int w0, w1, w2, w3, w4, w5, w6;
...
static ssize_t limits_show(struct device *dev, struct device_attribute *attr,
			   char *buf)
{
...
static ssize_t limits_store(struct device *dev, struct device_attribute *attr,
			    const char *buf, size_t count)
{
	int ret;
	int l0, l1, l2, l3, l4, l5;
...
static ssize_t bit_shifts_show(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	int b0, b1, b2;
...
static ssize_t bit_shifts_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t count)
{
	int ret;
...
static void do_afe_init(struct work_struct *work)
{
	struct afe_init_work *afe_init =
		container_of(work, struct afe_init_work, work.work);
...
err:
	pci_err(adev->fdev->pdev,
		"AFE initialization failed at index %d: %d", i, ret);
...
static void do_afe_sync(struct work_struct *work)
{
	struct afe_sync_work *afe_sync =
		container_of(work, struct afe_sync_work, work);
...
{
	struct afe3_dev *adev =
		container_of(inode->i_cdev, struct afe3_dev, cdev);
...
static int afe_cdev_fsync(struct file *file, loff_t start, loff_t end,
			  int datasync)
{
	struct afe3_dev *adev = file->private_data;
...
int target_fpga_platform_driver_probe(struct fpga_dev *fdev, dev_t fpga_devt,
				      struct class *device_class)
{
	...
	adev = devm_kzalloc(&fdev->pdev->dev, sizeof(struct afe3_dev),
			    GFP_KERNEL);
	if (!adev)
		return -ENOMEM;
	...
	if (IS_ERR(adev->device)) {
		...
		pci_err(fdev->pdev,
			"Error %d while trying to create target-afe", ret);
		goto err_device;
	}
	...
	queue_delayed_work(adev->workqueue, &afe_init->work,
...
void target_fpga_platform_driver_remove(struct fpga_dev *fdev,
					struct class *device_class)
{
	struct afe3_dev *adev;
