	...
	u64 fpga_base, u64 ram_base, u64 size)
{
	...
	for (retries = 0; retries < LINK_WAIT_MAX_IATU_RETRIES; retries++) {
		...
		if (val == PCIE_ATU_ENABLE) {
			pci_info(pdev, "iATU took %d retries", retries);
			return;
		}

		usleep_range(LINK_WAIT_IATU_MIN, LINK_WAIT_IATU_MAX);
	}
	pci_err(pdev, "iATU is not being enabled\n");
}
...
{
	pws->start = dmam_alloc_coherent(&pdev->dev, pws->size,
					 &pws->dma_handle,
					 GFP_KERNEL | __GFP_ZERO);
	if (pws->start == NULL) {
		pci_err(pdev, "Could not alloc %d bytes", pws->size);
		return false;
	}
	return true;
}
...
static int fpga_setup_irq(struct fpga_dev *fdev)
{
	...
	if (ret < 0) {
		pci_err(pdev, "Could not request MSI\n");
		return ret;
	}
	pci_info(pdev, "Enabled %d interrupts\n", ret);

	irq = pci_irq_vector(pdev, 0);
	ret = devm_request_irq(&pdev->dev, irq, handle_msi,
			       0, "fpga-data", fdev);
	if (ret) {
		pci_err(pdev, "Failed to request irq %d\n", irq);
		return ret;
	}
			      ...
			      size_t size, loff_t *offset)
{
	...
	fdev->unsent_start = (from_position + bytes_to_read)
			   & (fdev->data.size - 1);
...
static enum flash_type get_flash_type(struct pci_dev *pdev, u32 flash_type)
{
	if ((flash_type & 0x00ffff00) == 0x00ba1800)
		return FLASH_TYPE_N25Q;
	if ((flash_type & 0x0000ff00) == 0x00001800)
		return FLASH_TYPE_EPCQ;
	pci_err(pdev, "Invalid flash type: %x", flash_type);
	return FLASH_TYPE_N25Q;
}
			     ...
			     const struct pci_device_id *id)
{
	...
	if (pdev->vendor != PCI_VENDOR_ID_TARGET
		|| pdev->device != PCI_DEVICE_ID_TARGET_FPGA)
		return -ENODEV;
	...
	if (ret) {
		pci_err(pdev, "pci_enable_device() failed\n");
		goto err_managed;
	}

	ret = pcim_iomap_regions(pdev, 0x3, TARGET_FPGA_DRIVER_NAME);
	if (ret) {
		pci_err(pdev, "pcim_iomap_regions() failed\n");
		goto err_managed;
	}
	...
	if (pci_find_ext_capability(pdev, PCI_EXT_CAP_ID_ERR))
		...
		pci_info(pdev, "AER not supported\n");

	pci_set_master(pdev);

	flash_type = bar_read(&pdev->dev, FPGA_FLASH_TYPE);
	fdev->flash_type = get_flash_type(pdev, flash_type);
	...
	if (IS_ERR(fdev->device)) {
		...
		pci_err(pdev,
			"Error %d while trying to create target-fpga", ret);
		goto err_device;
	}
...
static int __maybe_unused fpga_driver_suspend(struct pci_dev *pdev,
					      pm_message_t state)
{
	struct fpga_dev *fdev;
...
static int __maybe_unused fpga_driver_resume(struct pci_dev *pdev)
{
	int ret;
	struct fpga_dev *fdev;
