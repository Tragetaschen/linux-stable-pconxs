...
if (ret < 0) return ret;
	...
	array = ssd1307fb_alloc_array(128 * 4, SSD1307FB_DATA);
	if (!array)
		return -ENOMEM;
	ssd1307fb_write_array(par->client, array, 128 * 4);
	kfree(array);
...
static int ssd1307fb_ssd1305_init(struct ssd1307fb_par *par)
{
	...
	ret = ssd1307fb_write_cmd(par->client, par->com_pins_configuration);
	if (ret < 0)
		return ret;
	if (ret < 0)
		return ret;
	...
	ret = ssd1307fb_write_cmd(par->client, par->column_offset);
	if (ret < 0)
		return ret;
